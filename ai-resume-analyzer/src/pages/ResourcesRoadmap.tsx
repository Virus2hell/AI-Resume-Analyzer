...
const ResourcesRoadmap = () => {
  ...
  const handleGenerate = async () => {
    ...
    // 1. Require auth before hitting the API
    if (!user) {
    ...
    try {
      ...
      setError(
        err.message || "Something went wrong while generating roadmap."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="section-container py-12">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-4 text-muted-foreground">
            Paste a company job description and get an AI-generated skill
            roadmap, projects and learning resources tailored to that role.
          </p>

          <div className="mt-8 card-base">
            <h2 className="text-lg font-semibold">Job Description</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              Include the full posting: responsibilities, required skills,
              qualifications, and about the role sections.
            </p>
            <textarea
          ...
          {!user && (
            <p className="mt-4 text-xs text-center text-muted-foreground">
              You can view this page without an account, but you need to sign
              up or log in to generate a personalized roadmap.
            </p>
          )}
        </div>
      </div>
